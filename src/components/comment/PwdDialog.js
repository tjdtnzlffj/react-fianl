...
const PwdDialog = ({ dialogOpen, dialogClose, addComment }) => {
			...
			<Dialog open={dialogOpen} onClose={dialogClose} aria-labelledby="form-dialog-title">
				...
				<DialogContent>
					...
					<TextField
						...
						type="password"
						fullWidth
						onKeyPress={(event) => { addComment(event, inputPwdRef.current.value); }}
				...
				<DialogActions>
					...
					<Button
						onClick={(event) => { addComment(event, inputPwdRef.current.value); }}
						color="primary">
						입력
					</Button>
				</DialogActions>
