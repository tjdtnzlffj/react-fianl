...
const CommentItem = ({ comment }) => {
	...
	return (
		<>
			...
			{
				modifyState ? <TextField defaultValue={comment.content} size="small" variant="outlined" inputRef={inputModifyingComment} onKeyPress={(event) => { change(event, inputModifyingComment.current.value); }} style={{ width: "100%" }} /> : <Typography key={comment.no} color="primary" style={{ textAlign: "left" }}>{comment.content}</Typography>
			}
